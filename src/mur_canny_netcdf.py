...
def canny_front_detection_1day(df, day_txt, base_path, thresh_min=120, thresh_max=220, apertureSize=5, sigma=5):
    ...
    canny_mask =np.ma.masked_where(mask_dilated==255, canny)   #Mask an array where a condition is met
    ...
    plt.savefig(os.path.join(base_path,'data/MUR_canny_netcdf/Canny_' + day_txt +'.jpg'))
    
    
    
    fn = os.path.join(base_path, 'data/MUR_canny_netcdf/Canny_' + day_txt + '.nc')
    ...
    print('var size after adding data', value.shape)
...
def main():
    ...
    exist_path = os.path.exists(os.path.join(base_path, 'data/MUR_canny_netcdf'))    #check if folder MUR_algorithm_daily_images exists in data folder
    if not exist_path:                                                                         #if doesn't exist
        os.makedirs(os.path.join(base_path, 'data/MUR_canny_netcdf'))                # create the folder
